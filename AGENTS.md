...
产品方向的非目标（含日期、结论与理由）已移入 **[docs/PRODUCT_DECISIONS.md](docs/PRODUCT_DECISIONS.md)**：
...
当请求是「同步/汇报工作区进度」时，交付物是**信息**：基线状态、待办入口、未决问题。
不是「挑队首的活开干」。本条来自 2026-09-12 的一次真实违约：把一个不明确的「继续」
解读为「开工 P0-4」，直接写了测试文件——而那批待办**明文规定要先过一道语义确认关
才能动代码**（`DELETION_MODEL.md` §7 ② 之前）。同会话内这已是多次越权。

约束：

- **消息里的动词就是全部授权。**「同步进度」「看一下」「评估」＝只读；「修」「实现」＝可写，
  且只写被点名的范围。
- **待办队列是清单，不是许可。** 任何队列条目在被明确点名开工之前，对当前会话都是
