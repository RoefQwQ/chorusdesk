...
| `scripting` | 辅助权限 | 点击关注弹窗时，在当前页面运行简短提取脚本，获取页面上公开展示的昵称与头像链接；抖音同步时向已打开的抖音标签页注入只读采集脚本；小红书**历史回溯**时向已登录的小红书标签页注入只读采集脚本（只读取页面已渲染的内容，不读取 Cookie、本地存储、请求头或设备标识，也不调用平台的签名接口）。 |
