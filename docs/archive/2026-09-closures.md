# Chorus 2026-09 已关闭条目与旧版队列（冻结的历史）

> 本文件由 `docs/PROJECT_PROGRESS_2026-09.md` 于 2026-09-12 **第二次收束**时拆出。
> 拆出的是该文「二、仍然存在的不足」与「四、后续优先级」的**上一版原文**，逐字保留，
> 作为「当时是怎么判断的、关掉时留下了什么证据」的记录。
>
> - 当前状态与**唯一**待办入口回到 [PROJECT_PROGRESS_2026-09.md](../PROJECT_PROGRESS_2026-09.md)
>   （现状速览 / 二 / 三 / 四）。
> - **不与代码同步**：其中的行数、测试计数、提交号、队列编号都是当时的快照；
>   与源码冲突时以源码为准。
> - 与 [2026-09-batches.md](2026-09-batches.md) 的分工：那份是**批次叙述**（第 1–33 批各批做了什么）；
>   这份是**条目台账**（每条「不足」与「优先级」是怎么关闭的、当时的实测证据是什么）。
> - **正文里已关闭的条目不再重复**；正文只留**现在**是什么、**还剩**什么。
>
> **为什么拆**：正文里混着 600 余行已完成内容，冷启动读者连续两轮读到其中的过期数字——
> 测试数写 651（实为 655）、「`urlParser` 8 处子串判定未改」（早已改走 `hostMatches`）、
> 「`AGENTS.md` 减负未做」（已做）、「队列 B32 根治未做」（已改为单一来源 + 守卫测试）。
> 长文里的陈旧段落会被读成现状，这是本仓库已记录三次的失效模式。
...
>
> **补充（2026-09-10，第三批）**：随后按界面逐项走查仍发现 14 项 UI/交互问题（见[批次记录 §一.13](2026-09-batches.md)），
...
- flex wrapping 和局部间距。
- 新增待确认：账号行加入平台名后，紧凑网格视图是否变得拥挤或换行（[批次记录 §一.16](2026-09-batches.md)）。
...
Referer 键必须是已声明平台域名。详见[批次记录 §一.14](2026-09-batches.md)。
...
已完成（详见[批次记录 §一.14](2026-09-batches.md)）：
...
### 11. ESLint — 已引入（2026-09-10，见[批次记录 §一.12](2026-09-batches.md) 与四.P5）
...
### 12. Dexie migration — 已进入永久 CI（2026-09-10，见[批次记录 §一.12](2026-09-batches.md) 与四.P2）
...
三轮走查共 14 项发现（`8465887` / `e2d9923` / `d4e031e`），全部修复，明细见[批次记录 §一.13](2026-09-batches.md)。
...
v3→v4 布尔转 `0|1` 契约（含墓碑快照与索引命中）被永久钉死。详见[批次记录 §一.12](2026-09-batches.md)。
...
首轮 77 错误全修零抑制（含 1 个真回归与 1 个真 bug）。详见[批次记录 §一.12](2026-09-batches.md)。
...
1. **RSS 卡片与阅读视图**（第十五/十七/十八/十九批项）。**根因链已全部定位并修复**：
   ...
   **新增 `Post.contentHtml`**（净化后的文章 HTML）+ `.article-body` 标签级排版；
   ...
   已在本地用真实源 + 构建产物渲染复核（见[批次记录 §一.30](2026-09-batches.md)）。
...
- **2026-09-12 审计批：[AUDIT_2026-09-12.md](../AUDIT_2026-09-12.md)**——P0/P1 全批 +
  P2 的 13–15、18–20 已实现；**P2-16 经用户决定不做**（不采用 PR-first）。
  **P2-17 仍有未做的一半**：**（b）`AGENTS.md` 减负是独立待办、顺序约束已满足**
  （把最长几条的事故叙述搬进 `AGENTS_CASES.md`、Non-goals 移出、规则压到 2–8 行）；
  **（a）待办迁 GitHub Issues 未决定，需用户拍板**。
  > **勘误（2026-09-12）**：本行此前把 P2-17 整条写成「已否决（用户决定）」——**是错的**。
  > 用户只否决了 PR-first（#16）；`AGENTS.md` 减负与「待办放哪」互不依赖
  > （`AGENTS.md` 里没有待办，它的 `Fix queue` 自述全部 DONE）。
  > 把用户没做过的决定写成用户的决定，比漏做更严重，故此处显式留痕。
  删除域的设计/不变量/实现清单在 [`DELETION_MODEL.md`](../DELETION_MODEL.md)（§9 是「实现了什么、在哪里」）。
  该批最有价值的方法论产出：**对抗性审查抓到了测试没抓到的 2 个真实缺陷**（v6 迁移丢归属字段、
...
#### 队列 B — 用户已批准或已列为不足（5 已完成；6/7 部分完成）

5. ~~**悬停预取**（用户已认可方案）~~ —— **已完成（2026-09-12）**：`usePlatformPrefetch.ts`
   + 侧栏 `@mouseenter`。每平台**一次**（会话内不重置）、首屏 36 条、磁盘缓存未绑定时
   直接跳过（`isReady()` 先探测，不产生 N 次失败写）、被更新的悬停取代即停止、逐条失败不回滚。
   5 例测试钉住这些边界（见 `tests/platformPrefetch.test.ts`）。
6. **二.4 `FetchError` 精度与重试**：宽范围 `try/catch` 仍会把解析/认证问题归为 network；
   `retryable` 尚未驱动实际重试。需按 HTTP/解析/schema/timeout/transport 分开分类，
   再实现受限指数退避。
   > **部分完成（2026-09-12，AUDIT P1-1/P1-3）**：`storage` 域已切开（写库失败不再归
   > network，且该 code 驱动平台冷却，分类错会冷却没被联系过的平台）；多源适配器可报
   > `degraded`。**剩余**：HTTP/解析/schema/timeout 的进一步细分与 401/403 专门归类，
   > 以及「按可重试性重试」——后者已明确否决（与规则 19 冷却冲突，见队列 B31）。
7. **二.5 备份校验不是完整 schema**：缺 timestamp 有限性、platform 枚举、跨表引用完整性、
   重复 ID、settings 字段范围、`isRead`/`isBookmarked` 必须为 `0|1` 等检查。
   > **已扩充（2026-09-12，AUDIT P0-4）**：`suppressions` 的必填字段校验
   > （`postId`/`platform`/`suppressedAt`）随格式 1.1 加入，版本号改为白名单
   > （`['1.0','1.1']`，不是相等判断）。**其余检查项仍未做**（timestamp 有限性、
...
B30. **Withny 平台整体移除**（2026-09-12，用户决定）——**已完成（提交 `0774e68`）**。现为 9 个平台。改动位点：适配器文件、
    ...
    静默失败。平台已去，问题随之消失；**该问题类已在 2026-09-12 根治**（前缀改为从
    `urlParser.GENERATED_NAME_PREFIXES` 单一来源派生 + 双向守卫测试 `tests/urlParser.test.ts`，
    另见审计 P1-5 与 AGENTS 规则 9）——所以这里没有遗留的「两份清单」需要复核。
...
B31. ~~**`FetchError.retryable` 是只写字段**~~ —— **已删除（2026-09-12，用户选 A）**。
    ...
    **为什么不做 B（接上重试）**：那会新增一条重试路径＝向刚失败的平台多发请求，与 AGENTS 规则 19
    ...
    立即重试确实有效，现在没有这样的数据。
B34. **bilibili 拒绝请求时，用户被告知「账号可能无投稿或已注销」**（2026-09-12，**已修**）。
    用户质疑「我开着 bilibili 页面就不会触发风控，肯定是你请求不对」——**实测证明他对、我的报告错**：
      · 无会话、无 Referer → **HTTP 412 + HTML** 体；
      · 无会话、带适配器那套头 → **HTTP 412 + `{"code":-412,"message":"request was banned"}`**；
      · **用户已登录的浏览器** → **HTTP 200 / code 0 / 13 条 item**。
    即 **412 是「未携带登录态」的响应**，不是泛泛的「风控」；`B站请求被拦截（风控）` 这句文案本身也不准确。
    **真正的缺陷**：`biliCodeError` 里 `-412 → rate_limit` 的映射**一直是对的，却永远走不到**——
    真实 412 是 HTTP 层失败，`if (res.ok && res.data)` 直接跳过解析，最后落到
    `fetchError('not_found', '未获取到B站内容（账号可能无投稿或已注销）')`。
    于是**任何未登录用户（或会话失效后）同步 bilibili，都会被告诉「这个账号没有投稿或已注销」**。
    已修：`businessCodeFromBody()` 从 HTTP 失败体里取业务码（体是 HTML 时返回 undefined，退回按状态码判断），
    feed 与 medialist 两处 HTTP 失败都据此设置 `lastDynamicCode` / `lastMediaCode`。
    实测修复后同一输入返回 `rate_limit · B站请求被拦截（风控），请稍后重试`。
    **我的方法错误（比缺陷本身更值得记）**：我把这个发现写成测试时，喂的是
    `{ok: true, code: -412}` —— **现实中不存在的组合**（真实 412 是 HTTP 状态、`ok: false`），
    于是断言建立在一个假输入上，既没复现缺陷、也没验证修复。已改为真实形态（含 HTML 体那一支），
    8 例全过。**「我构造了一个输入并观察到了行为」不等于「我复现了那个场景」。**
B26. ~~**`PLATFORMS.md` §2.2 的 Twitter 建议此前是空头支票**~~ —— **已验证（2026-09-12，真机日志）**。
    用户在自己已登录的浏览器上跑了一次全量同步，四个推特渠道**全部走标签页路径、全部成功**，
    且都是「复用已打开的 x.com 标签页」：
    `[DEBUG] sw twitter: 标签页路径：复用已打开的 x.com 标签页` →
    `[DEBUG] sw twitter: 标签页路径采集成功 | tab …`（连续 4 次，同一标签页）。
    这同时证明了 §2.2 那句「可先在浏览器打开目标博主主页，扩展会优先复用当前活跃标签页」——
    在该路径修好之前它做不到（函数一进页面就 `ReferenceError`），现在成立。
    **另一条附带结论**：本次日志里**直连路径一次都没被走到**，因为主路径全部成功（这正是设计顺序）。
    直连作为兜底，其可用性由修复前的历史日志佐证（那时页面路径静默失败、每次都落到直连且能同步成功）。
    **顺带修掉一个我自己造成的日志缺陷**：处理器把「请求成功」记成「采集成功」，
    于是受保护账号（响应合法但没有任何推文条目）会出现
    `标签页路径采集成功` 紧接 `同步失败（parse）· 未返回任何推文条目` 的自相矛盾。
    现已区分三种状态：请求失败 → `warn 标签页路径采集失败`；请求成功但无条目 →
    `warn 标签页路径：请求成功，但响应中没有推文条目`；确有条目 → `debug 标签页路径采集成功`。
    测试改为按**载荷形态**分两个用例钉住这对区别（默认桩返回空 timeline＝受保护账号形态）。
...
8. **发布范围**（二.10）：**已决定 —— 不上架**（2026-09-11，用户答复；决定记录在
   `docs/PUBLISHING.md` 顶部）。三件决定全部落地：① **不上架**（放弃商店，保留 GitHub 发布，
   换到零审核延迟与零政策风险，代价是无自动更新）；② **抖音表述口径**见该文件 §4.3 的如实写法；
   ③ **第 4 位版本机制已实现**（`CHORUS_STORE_REVISION`，默认不生效、产物与从前逐字节一致，
   仅在被驳回需重提时使用）——见 `PUBLISHING.md` §5.3 与 `wxt.config.ts`。
   材料（清单字段、逐权限理由、审查员自查三项、升级/降级说明、抖音与 X 的能力边界）
   **保留为草稿**，将来若重启上架可直接用；§1.4 的 5 条合规缺口已随「不上架」失效。
