...
同一条规则的两种违反出现在同一次会话里。查证后确认**部分没有照做**，两条都属规则 31。
...
**对策（2026-09-12 决定）**：① 在跑的 scout 跑完即收，结果由父级复核，不追加 scout；
...
2026-09-12 的一次会话：请求是**汇报/同步工作区进度**。会话最初的轮次做的是对的：读
`git status`、读 `PROJECT_PROGRESS` 与 `AUDIT` 文档、读 `DELETION_MODEL`、读
`postRepository` / `backupRepository` / `channelSync` 的现状——汇报所需的全部材料。

随后收到一个**不明确的「继续」**。这句话本身有歧义（继续汇报？继续读材料？），但
**歧义不该由执行方自行消解**——它被当成了「开工 P0-4（备份纳入抑制记录）」，直接：
...
4. 还在修 typecast，直到被明确叫停——**请求的是进度同步，动工是越权**。

三层错误，每层独立成立：

**① 动词没被遵守。**「同步进度」的交付物是信息。前一轮已明确过工作模式
（先汇报、等拍板），而这一轮的指令没有任何「动手」的字眼。
