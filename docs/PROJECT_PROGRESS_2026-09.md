...
1. ~~**文档收口（收益最大，先做）**~~ —— **已完成（2026-09-11）**：
   `docs/PROJECT_PROGRESS_2026-09.md` 2660 → 545 行，只留现状速览 / 二 / 三 / 四；
   原「结论 + 一（45 节，含第 1–33 批）+ 最终评价」**逐字**移入
   `docs/archive/2026-09-batches.md`（2126 行，冻结）；正文中 10 处「一.x」引用改指归档。
   `AGENTS.md` 973 → 779 行：12 条长案例（16/17/19/21/23/24/26/27/28/29/30/31）压成
   「规则 + 规范性约束 + 附录指针」，拆分前原文逐字保存在 `docs/AGENTS_CASES.md`
   （555 行）；规则编号未变。拆分时一并修掉四处过期事实：`CreatorsView` 行数口径
