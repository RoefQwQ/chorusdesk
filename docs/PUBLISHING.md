...
审核时长从几小时到数周（复杂扩展、高峰期更久）。本项目 10 个平台的采集方式**全部**建立在
...
每次平台改版要跑两遍完整验证，而 10 个平台的维护成本已经很高。更重要的是风险不在某个可摘除的
...
5. **是否要标记为 Mature，需要按实际内容判断。** *Mature & Sexually Explicit Material* 第 5 条：
   ...
   相关代码事实：设置里有一个 `enableR18Blur`（默认 `true`）——**但它没有任何读取方**，
   是个死设置，见 §1.5 的代码发现。也就是说产品当前**没有**模糊能力，能不能看到 R18 内容
   完全取决于用户自己登录的账号能看到什么。按这个事实判断，需要标记 Mature 的可能性是**高**的，
   但这属于**推断**，最终以实际抓取到的内容为准。

### 1.5 本次核对顺带发现的代码问题（与上架无关，也应修）

- **`enableR18Blur` 是死设置。** 全仓 3 处出现：类型声明、`DEFAULT_SETTINGS` 默认值、
  `useDashboardData` 的默认值。**没有任何组件或逻辑读取它**，设置页的布尔开关只支持
  `enableAutoSync` 与 `hideReposts`（`updateBooleanSetting` 的参数是这两个的联合类型）。
  一个「默认开启」的模糊开关存在但不起作用，比没有这个设置更糟：它读起来像一项已实现的
  安全/合规能力。
