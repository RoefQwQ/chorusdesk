...
- **`enableR18Blur` 已删除（2026-09-11）。** 它此前有三处声明（类型、`DEFAULT_SETTINGS`、
  ...
  因此设置、类型与默认值一并移除（见 `docs/archive/2026-09-closures.md` 的 B28）。
...
> **工作流代码取自被 tag 的那个提交，不是 master。** 这是 2026-09-13 实际踩到的：
> v1.0.0 指向 `005a276`，而修复发布 5xx 的提交 `1cf7ab5` 在它**之后**——于是那个 tag 触发
> 跑的是旧的单行 `gh release create … --generate-notes`：四次运行（`77ec94a` 与 `005a276`）
> 全部失败，报的是 `HTTP 500 / 502`。**改了发布流程之后，旧 tag 不会受益**；要验证新工作流，
> 只能等下一个 tag。
>
> 附带一个必须知道的事实：**那四次全部失败，但 v1.0.0 的 Release 最终是已发布状态**
> （`created_at` 09:25:11，早于最后一次运行的 09:25:14）。也就是说，**发布是被工作流之外
> 的动作补完的**——工作流的绿灯与「有没有发布成功」不是同一件事，判据只能是 Release 页面本身。
>
> 反过来也要小心：Actions 里唯一那次**绿色**的 Release 运行是 `workflow_dispatch`，
> 它跳过了全部 `if: startsWith(github.ref, 'refs/tags/')` 的步骤——跑了门禁，**没有发布任何东西**。所以
> 「Release 通过了」这句话在手工触发下不成立；要看的是那次运行是否为 `push` 事件且带着 tag。
>
> **另一件必须会看的事：步骤红了 ≠ 发布失败。** v1.0.1 那次步骤报红，但 Release 本身是成功的
> （附件在、无残留草稿、正文正确）。判据永远是 Release 页面，不是那个红叉。
> 原因是 GitHub 用 `bash -e` 跑 `run:`，而 `-e` 会让**任何**失败命令终止脚本——包括赋值里的
> 命令替换。发布步骤是个重试循环，里面的探测读取若没有 `|| true`，第一次瞬时 5xx 就会**在循环
> 有机会重试之前**杀掉整个步骤；日志上的特征是：**既没有 `published with asset`，也没有
> `::error::`，直接退出码 1**。已修（`22b5baf`），并加了三个断言钉死。
>
> 注意这类错误的**继承性**：修在某个提交上，只有**之后**打的 tag 才带上它。v1.0.1 的 tag 树里
> 没有护栏，所以**重跑那次运行仍会失败**——重跑不解决问题，它只是让红叉再出现一次。
