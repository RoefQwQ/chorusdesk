...
> 身份必须**跨 channel 重建存活**，见 §3；
> ② 取关时**要提示**该创作者下有多少条删除仍保持生效，见 §6 问题 5；
> ③ v6 之后 `deletedPostIds`：**删除**（§6 问题 1），接受迁移不可回退；
> ④ suppression 冗余存储 `platform` 字段（§6 问题 2）；
> ⑤ 手动「清理 N 天前数据」删掉的动态**不**建立 suppression（§6 问题 6）。
>
> **§6 全部六个问题均已作答（2026-09-12）**——设计语义已冻结，可以按 §7 的顺序开工。
...
version(6).stores({
  ...
  // platform 从旧行冗余取值（§6 问题 2 已决：不靠 postId 前缀推断）。
  ...
  // v6 之后 deletedPostIds 整表删除（§6 问题 1 已决，2026-09-12）。
  // v5 的后向兼容不存在：Dexie 迁移是单向的，用户无法降级回 v5。
});
```
...
## 6. 六个问题（2026-09-12 全部已作答）

外部审查列出的四个，加上我核对身份模型时发现的两个。**均已由用户拍板，语义冻结。**

| # | 问题 | 影响 |
|---|---|---|
| **1** | **`deletedPostIds` 这张表在 v6 后留不留？** ——**已决定：删除（2026-09-12）。** 不留过渡表；接受迁移不可回退（Dexie 单向，无法降级回 v5），出问题时以重建库 + 导入备份兜底（备份自 P0-4 起含抑制记录）。理由：本仓库明令禁止维护两套实现；旧表只在本地，重建成本可接受 | 已决定 |
| **2** | **`platform` 冗余存储 vs 从 `postId` 前缀推断？** ——**已决定：冗余存储（2026-09-12）。** 新 suppression 表显式存 `platform` 字段，不依赖 `_` 分隔的 ID 命名约定（`bilibili_video_<bvid>` 本身含下划线，前缀切分脆弱）；迁移时从旧行取值，该值同时可作迁移校验 | 已决定 |
...
| **5** | **取关时是否提示用户？** ——**已决定：提示（2026-09-12）。** 用户已定"删过的不回来"，而 suppression 在取关那一刻**静默存活**，用户无从得知。取关确认框需说明该创作者下有多少条删除仍保持生效（例如「该创作者下你删过的 N 条动态仍保持删除状态，重新关注后也不会再出现」）。**若 N = 0 则不提示**，避免为常态弹无意义的说明 | 已决定，待实现 |
| **6** | **手动「清理 N 天前数据」与 suppression 的关系？** ——**已决定：不建立 suppression（2026-09-12）。** 该清理是**用户手动触发**的存储维护（设置页「清理 60 天前数据」按钮 + 确认框；全仓无任何自动调用，后台 alarm 只有自动同步），但它删的是"未收藏的旧内容"，属存储维护而非"我不要这条内容"的意图——重装或重新同步后仍可重新出现。**当前实现已经是这样**（`cleanupOldPosts` 直接 `bulkDelete`，不写任何记录），本条决定的作用是**钉死现状**，防止改造时顺手改动 | 已决定 |

**§6 状态：六个问题全部已作答（2026-09-12），语义冻结。**
问题 3 为核实结论（非选择），问题 4 的影响仅限显示层文案（不阻塞迁移）。
无剩余阻塞项，可按 §7 顺序开工。
...
```
① 本文件（状态表 + 不变量 + 迁移映射）        ← 已完成
        ↓  用户确认语义                      ← 已完成（2026-09-12，§6 六问全答）
② 不变量测试（先红：证明当前违反 I2/I3/I5/I10/I11/I12）   ← 下一步，等用户发令
        ↓
③ Dexie v6 迁移 + 两表实现（删旧表；suppression 带 platform 字段）
        ↓
...
> **② 是「等发令」，不是「自动开始」**：语义确认解开的是「可以做」，不是「现在做」。
> 开工需用户明确点名（`AGENTS.md` 规则 32）。
