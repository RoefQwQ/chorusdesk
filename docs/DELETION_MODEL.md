...
> ③ v6 之后 `deletedPostIds` 留不留：**待定**（§6 问题 1）——它只阻塞 ③ 迁移，不阻塞 ② 不变量测试。
...
**问题 1（`deletedPostIds` 在 v6 后留不留）状态：待定（2026-09-12）。**
用户未决。两个方向的代价已在表内列明：留 = 过渡期但两套并存（本仓库明令禁止维护两套导入路径/实现）；
删 = **迁移不可回退**（Dexie 迁移单向，用户无法降级回 v5）。
**在它定下来之前只做 ②（不变量测试），不做 ③（迁移）**——② 不依赖这个答案。
