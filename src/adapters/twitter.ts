...
export const twitterAdapter: PlatformAdapter = {
  ...
  async fetchLatest(channel: Channel, limit: number = 10, options?: FetchOptions): Promise<FetchResult> {
    ...
    try {
      ...
      if (res.tweetData) {
        // A direct call to the parser above. This used to read
        // `this.parseGraphQLResult` and bail when it was missing — a dance that
        // existed only because the method was declared OPTIONAL on the shared
        // `PlatformAdapter` interface, where nobody else could ever call it. The
        // optionality meant this function had to prove its own existence to
        // itself, and a missing parser became a runtime `parse` error rather
...
/**
 * Normalize a `FETCH_TWITTER_TIMELINE` response into `Post[]`.
 *
 * Module-scoped rather than a member of `twitterAdapter`, because it was NEVER
 * part of the adapter contract — see the note on `PlatformAdapter` in
 * `types.ts`. Exported by name so tests can call it directly without reaching
 * through an optional interface member with a non-null assertion.
 */
export function parseGraphQLResult(
  channel: Channel,
  tweetData: unknown,
  userData: unknown,
  limit: number,
